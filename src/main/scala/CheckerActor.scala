...
class CheckerActor (val id:Int, val terminaux:List[Terminal], electionActor:ActorRef) extends Actor {
    ...
    def receive = {
         ...
         // Initialisation
        case Start => {
             context.system.scheduler.schedule(0 milliseconds, 100 milliseconds, self, CheckerTick)
        }
